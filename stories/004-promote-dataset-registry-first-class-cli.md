...
Below are snippets of datasets that could be added to the registry.
They are split into groups of linear and nonlinear datasets.
Some of these are implemented, some are not. Some are good for certain types of experiments, some are not, like regression vs classification vs denoising vs sparsity vs etc.
