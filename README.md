...
[![ReleaseStaticBadge](https://img.shields.io/badge/Version-0.5.0-darkgreen?style=for-the-badge)](https://github.com/TennisGazelle/HexNets/releases/latest)
...
| `hexnet train` | Train hex or MLP; writes under `runs/` (optional `--run-note`, `--run-tags`; optional `--dataset-noise` / `--dataset-noise-mu` / `--dataset-noise-sigma` for synthetic data). Start from an existing `runs/.../config.json` shape with `-rc` / `--run-config <path>` or `--run-config-json '<json>'`; any other CLI flags you pass override that template (omit a flag to keep the value from the JSON). Incompatible with `--run-dir` (resume). For **hex** only, optional `--epr` / `--epochs-per-rotation` and `--ro` / `--rotation-ordering` are validated and stored in `model_metadata` as `epr` / `ro` (rotation training behavior is not implemented yet). |
